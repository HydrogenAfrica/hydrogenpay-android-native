...
interface PayByCardApiService {
    ...
    @POST("card/get-customer-tokenized-cards")
    suspend fun getSavedCards(
        @Body getCardProviderRequestBody: GetSavedCardsRequestDTO
    ): Response<HydrogenServerResponseResource<List<GetSavedCardsResponseDTO>>>

    @POST("Payment/delete-saved-card")
    suspend fun deleteASavedCards(
        @Body deleteSavedCardRequestDTO: DeleteSavedCardRequestDTO
    ): Response<HydrogenServerResponseResource<Any>>
}